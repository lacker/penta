...
pub(in crate::card::sets) static ESHKI_TEMUR_S_ROAR: CardRecord = CardRecord::new(
    "Eshki, Temur's Roar",
    "ff9aa863-8773-452d-946c-ae334c632e11",
    "Billy Christian",
    CardRules::new_creature(mana_cost!("{G}{U}{R}"), &["Human", "Warrior"], 2, 2)
        .with_supertype(CardSupertype::Legendary)
        .with_abilities(&[AbilityDef::triggered(
            "Whenever you cast a creature spell, put a +1/+1 counter on \
             Eshki. If that spell's power is 4 or greater, draw a card. \
             If that spell's power is 6 or greater, Eshki deals damage \
             equal to Eshki's power to each opponent.",
            ...
            EffectDef::Sequence(&[
                ...
                EffectDef::IfCondition {
                    ...
                    then: &EffectDef::damage(EffectRecipientDef::Opponent, ValueDef::SourcePower),
                },
            ]),
        )]),
);
...
pub(in crate::card::sets) static AINOK_STRIKE_LEADER: CardRecord = CardRecord::new(
    "Ainok Strike Leader",
    "2d1102e4-447a-4b40-8131-5b36b8979fe0",
    "Alexander Mokhov",
    CardRules::new_creature(mana_cost!("{1}{W}"), &["Dog", "Warrior"], 2, 2).with_abilities(&[
        AbilityDef::triggered(
            "Whenever you attack with this creature and/or your \
             commander, for each opponent, create a 1/1 red Goblin \
...
// Audit: unsupported — Conditional mode counts and granted flashback are supported, but the
// first mode needs simultaneous optional whole-hand discard choices in APNAP order. Separate
// May effects would resolve one player's discard and draw before the other chooses.
pub(in crate::card::sets) static WILL_OF_THE_JESKAI: CardRecord = CardRecord::new(
    "Will of the Jeskai",
