...
static int check_standard_game(void) {
    ...
    int valid = strstr(observation, "\"format\":\"isd-m14-standard\"") != NULL;
    penta_string_free(observation);
    penta_free(game);
    if (!valid) {
...
int main(void) {
    ...
    char *standard_decks =
        penta_deck_names_for_format_json("isd-m14-standard");
    ...
    char *standard_catalog =
        penta_catalog_json_for_format("isd-m14-standard");
    if (!standard_catalog ||
        !strstr(standard_catalog, "\"format\":\"isd-m14-standard\"") ||
