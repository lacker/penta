...
impl Game {
    pub(super) fn payment_object(
        ...
        match purpose {
            ...
            ManaPaymentPurpose::Resolving { source, .. } => self
                .battlefield
                .iter()
                .find(|permanent| permanent.card.id == *source)
                .map(|permanent| (self.trigger_event_object(permanent), false)),
    ...
    pub(super) fn mana_can_pay_for(&self, mana: Mana, purpose: &ManaPaymentPurpose) -> bool {
        mana.restrictions
            ...
            .all(|restriction| match restriction {
                ...
                ManaRestrictionDef::NamedPayment(mechanic) => {
                    matches!(purpose, ManaPaymentPurpose::Resolving { mechanics, .. } if mechanics.contains(mechanic))
                }
                ManaRestrictionDef::Special(_) => false,
            })
            && match purpose {
                ManaPaymentPurpose::Resolving { snow: true, .. } => mana
                    .source
                    .and_then(|source| {
                        self.battlefield
                            .iter()
