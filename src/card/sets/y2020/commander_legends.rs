...
// CMR 3 — Akroma's Will
pub(in crate::card::sets) static AKROMA_S_WILL: CardRecord = CardRecord::new(
    "Akroma's Will",
    "c281997b-1566-4469-a14c-6645f81ab023",
    "Antonio José Manzanedo",
    CardRules::new_instant(mana_cost!("{3}{W}")).with_abilities(&[AbilityDef::modal_spell(
...
pub(in crate::card::sets) static HULLBREACHER: CardRecord = CardRecord::new(
    ...
    CardRules::new_creature(mana_cost!("{2}{U}"), &["Merfolk", "Pirate"], 3, 2).with_abilities(&[
        abilities::flash(),
        AbilityDef::defined_replacement(
            "If an opponent would draw a card except the first one they \
             draw in each of their draw steps, instead you create a \
             Treasure token. (It\'s an artifact with \"{T}, Sacrifice \
            ...
            ReplacementEffectDef::Sequence(&[
                ...
                ReplacementEffectDef::Perform(&EffectDef::CreateToken(CreateTokenDef::new(
                    TokenDef::Literal(TREASURE_TOKEN),
                ))),
            ]),
        ),
    ]),
...
// Audit: unsupported — Combat damage has a grouped player-damage event, but noncombat damage
// does not. This trigger must group simultaneous Pirate damage of either kind; a combat-only
// declaration would omit the noncombat half.
pub(in crate::card::sets) static MALCOLM_KEEN_EYED_NAVIGATOR: CardRecord = CardRecord::new(
...
// Audit: unsupported — Needs the Partner deck-construction permission and an entry-copy
// exception that retains every other ability printed on the source.
...
// Audit: unsupported — Needs an opponent-search replacement that transfers control of the
// searched library and redirects the found card's destination.
pub(in crate::card::sets) static OPPOSITION_AGENT: CardRecord = CardRecord::new(
    "Opposition Agent",
...
// Audit: unsupported — Casting costs cannot combine a variable additional sacrifice group with
// a two-mana discount per selected permanent. Turn history also has no count of other artifacts
// or creatures sacrificed this turn.
pub(in crate::card::sets) static DARGO_THE_SHIPWRECKER: CardRecord = CardRecord::new(
...
// Audit: unsupported — Command-zone cards can be put onto the battlefield, but the ordinary
// battlefield-exit procedure ignores a Command destination. The mandatory delayed return
// therefore cannot move the commander back; replacing it with an optional commander return
// through exile or hand would change the printed effect.
pub(in crate::card::sets) static HELLKITE_COURSER: CardRecord = CardRecord::new(
    "Hellkite Courser",
...
// Audit: unsupported — Needs Encore, including a graveyard activation, one token copy attacking
// each opponent, and delayed sacrifice.
pub(in crate::card::sets) static IMPULSIVE_PILFERER: CardRecord = CardRecord::new(
    "Impulsive Pilferer",
...
pub(in crate::card::sets) static JESKA_S_WILL: CardRecord = CardRecord::new(
    "Jeska's Will",
...
pub(in crate::card::sets) static KRARK_THE_THUMBLESS: CardRecord = CardRecord::new(
    "Krark, the Thumbless",
    "06a981cd-1951-438e-95c9-68294795638e",
    "Mathias Kollros",
    CardRules::new_creature(mana_cost!("{1}{R}"), &["Goblin", "Wizard"], 2, 2)
        .with_supertype(CardSupertype::Legendary)
        .with_abilities(&[
            AbilityDef::triggered(
                "Whenever you cast an instant or sorcery spell, flip a coin. \
                 If you lose the flip, return that spell to its owner's \
                 hand. If you win the flip, copy that spell, and you may \
                ...
                EffectDef::FlipCoin {
                    ...
                    on_loss: &EffectDef::move_to_zone(
...
pub(in crate::card::sets) static ROGRAKH_SON_OF_ROHGAHH: CardRecord = CardRecord::new(
...
// Audit: unsupported — Needs simultaneous secret integer choices, highest and lowest
// comparison, and the resulting per-player damage, discard, and draw.
pub(in crate::card::sets) static WHEEL_OF_MISFORTUNE: CardRecord = CardRecord::new(
    "Wheel of Misfortune",
...
pub(in crate::card::sets) static APEX_DEVASTATOR: CardRecord = CardRecord::new(
    "Apex Devastator",
...
// Audit: unsupported — Needs protection dynamically derived from colors outside the
// controller's commander color identity, plus commander-specific equip.
pub(in crate::card::sets) static COMMANDER_S_PLATE: CardRecord = CardRecord::new(
    "Commander's Plate",
...
pub(in crate::card::sets) static JEWELED_LOTUS: CardRecord = CardRecord::new(
    ...
    CardRules::new_artifact(mana_cost!("{0}")).with_ability(AbilityDef::activated_mana(
        "{T}, Sacrifice this artifact: Add three mana of any one \
         color. Spend this mana only to cast your commander.",
...
pub(in crate::card::sets) static REJUVENATING_SPRINGS: CardRecord = CardRecord::new(
...
pub(in crate::card::sets) static SPECTATOR_SEATING: CardRecord = CardRecord::new(
    "Spectator Seating",
...
pub(in crate::card::sets) static VAULT_OF_CHAMPIONS: CardRecord = CardRecord::new(
    "Vault of Champions",
...
// CMR 361 — War Room
// Audit: unsupported — Needs the number of colors in the controller's commanders' color
// identity as a life-payment value.
pub(in crate::card::sets) static WAR_ROOM: CardRecord = CardRecord::new(
    "War Room",
...
pub(in crate::card::sets) static KEDISS_EMBERCLAW_FAMILIAR: CardRecord = CardRecord::new(
    "Kediss, Emberclaw Familiar",
    "23766fa0-e673-46c7-a29f-3fb140844b1c",
    "Jesper Ejsing",
    CardRules::new_creature(mana_cost!("{1}{R}"), &["Elemental", "Lizard"], 1, 1)
        .with_supertype(CardSupertype::Legendary)
        .with_abilities(&[
            AbilityDef::triggered(
...
// CMR 669 — Port Razer
// Audit: unsupported — Needs per-turn memory of which players this creature attacked, together
// with an additional-combat insertion.
pub(in crate::card::sets) static PORT_RAZER: CardRecord = CardRecord::new(
    "Port Razer",
...
pub(in crate::card::sets) static TRAINING_CENTER: CardRecord = CardRecord::new(
    "Training Center",
...
pub(in crate::card::sets) static UNDERGROWTH_STADIUM: CardRecord = CardRecord::new(
    "Undergrowth Stadium",
...
pub(in crate::card::sets) static CARDS: &[&CardRecord] = &[
    &AKROMA_S_WILL,
    &HULLBREACHER,
    &MALCOLM_KEEN_EYED_NAVIGATOR,
    &SAKASHIMA_OF_A_THOUSAND_FACES,
    &OPPOSITION_AGENT,
    &DARGO_THE_SHIPWRECKER,
    &HELLKITE_COURSER,
    &IMPULSIVE_PILFERER,
    &JESKA_S_WILL,
    &KRARK_THE_THUMBLESS,
