...
fn parse_observed_basic_land_type_substitution(
    value: Option<&Value>,
) -> Option<(BasicLandType, BasicLandType)> {
    let pair = value?.as_array()?.as_slice();
    let [from, to] = pair else { return None };
    Some((
        BasicLandType::from_subtype(from.as_str()?)?,
        BasicLandType::from_subtype(to.as_str()?)?,
    ))
}
