...
has callable `attach`, `choose`, `next`, `inspect_ref`, and `retry` tools.
...
After an uncertain submission, use `retry({connection})` or repeat the identical
retained ticket and options. Resolve it before another move. Do not change a
submitted ticket's options. After a definite stale error, refresh and choose
again from the new view. A process restart loses local tickets; reattach.

Use exact `inspect` and `play` as the protocol fallback, described in
`docs/bot-sessions.md`. Report engine or information discrepancies with visible
position, revision, ticket and receipt context; omit seat credentials.
