...
pub(in crate::card::sets) static FAERIE_ARTISANS: CardRecord = CardRecord::new(
    "Faerie Artisans",
    ...
    CardRules::new_creature(mana_cost!("{3}{U}"), &["Faerie", "Artificer"], 2, 2).with_abilities(
        &[
            ...
            AbilityDef::triggered(
                "Whenever a nontoken creature an opponent controls enters, \
                 create a token that's a copy of that creature except it's \
                 an artifact in addition to its other types. Then exile all \
                 other tokens created with this creature.",
                ...
                EffectDef::BindObjects(BindObjectsDef {
                    source: ObjectCollectionSourceDef::ObjectSet(ObjectSetDef::TokensCreatedBy(
                        ObjectRefDef::Source,
                    )),
                    binding: Binding!("artisans_previous"),
...
pub(in crate::card::sets) static KRAUM_LUDEVIC_S_OPUS: CardRecord = CardRecord::new(
...
pub(in crate::card::sets) static SILAS_RENN_SEEKER_ADEPT: CardRecord = CardRecord::new(
    "Silas Renn, Seeker Adept",
    "4e3fe912-1374-47c7-b73f-89ef55c479c1",
    "Joseph Meehan",
    CardRules::new_artifact_creature(mana_cost!("{1}{U}{B}"), &["Human"], 2, 2)
        .with_supertype(CardSupertype::Legendary)
        .with_abilities(&[
            abilities::deathtouch(),
            AbilityDef::triggered_with_targets(
                "Whenever Silas Renn deals combat damage to a player, choose \
                 target artifact card in your graveyard. You may cast that \
                 card this turn.",
            ...
            AbilityDef::deck_construction(
                "Partner",
                DeckConstructionDef::Partner,
                "Commander pairing is chosen before the game.",
            ),
        ]),
);

// C16 45 — Tana, the Bloodsower
pub(in crate::card::sets) static TANA_THE_BLOODSOWER: CardRecord = CardRecord::new(
    "Tana, the Bloodsower",
    "a3d8d64f-a403-42a7-881b-4f70e9fe15a2",
    "Magali Villeneuve",
    CardRules::new_creature(mana_cost!("{2}{R}{G}"), &["Elf", "Druid"], 2, 2)
        .with_supertype(CardSupertype::Legendary)
        .with_abilities(&[
            abilities::trample(),
            AbilityDef::triggered(
                "Whenever Tana deals combat damage to a player, create that \
...
pub(in crate::card::sets) static THRASIOS_TRITON_HERO: CardRecord = CardRecord::new(
    "Thrasios, Triton Hero",
    "21e27b91-c7f1-4709-aa0d-8b5d81b22a0a",
    "Josu Hernaiz",
    CardRules::new_creature(mana_cost!("{G}{U}"), &["Merfolk", "Wizard"], 1, 3)
        .with_supertype(CardSupertype::Legendary)
        .with_abilities(&[
            AbilityDef::activated(
                "{4}: Scry 1, then reveal the top card of your library. If \
                 it's a land card, put it onto the battlefield tapped. \
                    ...
                    EffectDef::BindObjects(BindObjectsDef {
                        ...
                        then: &EffectDef::Sequence(&[
                            ...
                            EffectDef::ForEachInBinding {
                                objects: Binding!("thrasios_top"),
                                binding: Binding!("thrasios_card"),
                                effect: &EffectDef::IfElseCondition {
                                    ...
                                    then: &EffectDef::WithBattlefieldArrival {
                                        effect: &EffectDef::move_to_zone(
                                            EffectRecipientDef::objects(ObjectSetDef::One(
                                                ObjectRefDef::Binding(Binding!("thrasios_card")),
                                            )),
...
// Audit: unsupported — Turn history does not distinguish players dealt combat damage from
// players dealt noncombat damage; querying current attackers would lose earlier combat history.
pub(in crate::card::sets) static TYMNA_THE_WEAVER: CardRecord = CardRecord::new(
    "Tymna the Weaver",
...
// Audit: unsupported — No value counts the distinct colors represented among controlled
// permanents for the Equipment’s continuous power/toughness bonus.
pub(in crate::card::sets) static CONQUEROR_S_FLAIL: CardRecord = CardRecord::new(
    "Conqueror's Flail",
...
// C16 56 — Ash Barrens
pub(in crate::card::sets) static ASH_BARRENS: CardRecord = CardRecord::new(
    "Ash Barrens",
