...
use crate::card::{
    ...
    ActivationTimingDef, AddManaEffectDef, AlternativeCastKindDef, ArrivalAttachmentDef,
    BasicLandType, CardArt, CardRules, CardSet, CardSupertype, CardType, CardTypeSet,
    ChoiceVisibilityDef, ChooseDef, ColorSet, ComparisonDef, DiscardSelectionDef, DividedTotal,
    EffectDef, EffectPaymentCostDef, EffectRecipientDef, InstalledTriggerDef, KeywordAbility,
    ...
    ResolvedEffectDurationDef, SpellAdditionalCostDef, TargetChooserDef, TriggerConditionDef,
...
macro_rules! bounce_land_rules {
    ($returned_subtype:literal, $produced:expr, $trigger_text:literal, $mana_text:literal $(,)?) => {
                    ...
                    EffectPaymentDef {
                        ...
                        cost: EffectPaymentCostDef::MovePermanentMatching {
                            object: ObjectPredicateDef::All(&[
                                ObjectPredicateDef::Subtype($returned_subtype),
...
// VIS 3 — Equipoise
// Audit: metadata-only — Needs repeated excess-count choices by permanent type plus linked phasing that lasts until the chosen player's next untap step.
...
// Audit: metadata-only — Needs grouping battlefield permanents by shared name for the initial duplicate sweep and each later nonbasic entry.
...
// Audit: metadata-only — Target legality cannot yet identify an unblocked creature for the duration-scoped combat-damage prevention effect.
...
// Audit: metadata-only — Needs a resolving choice of any damage source and a prevention continuation that reuses the exact prevented amount against a red source's controller.
...
// Audit: metadata-only — Flanking is not yet a removable keyword ability, and the activated clause also needs the full set of nonflanking creatures blocking this creature.
...
pub(in crate::card::sets) static MIRACULOUS_RECOVERY: CardRecord = CardRecord::new(
    ...
    crate::card::CardSet::Visions,
    CardRules::new_instant(mana_cost!("{4}{W}")).with_ability(
        AbilityDef::spell_with_targets(
...
// VIS 15 — Peace Talks
// Audit: metadata-only — Needs attack and targeting prohibitions whose duration spans the current turn and the entire next turn.
...
// VIS 17 — Remedy
// Audit: metadata-only — Divided target shares currently drive damage amounts, not a five-point prevention shield divided among mixed permanent and player targets.
...
// Audit: metadata-only — Needs a resolving choice of any damage source for a one-shot shield protecting the ability's controller.
...
// Audit: metadata-only — Its activated phase-out is available, but flanking is not yet a removable keyword ability.
...
// VIS 23 — Tithe
// Audit: metadata-only — Search maximum cannot yet conditionally become two by comparing the target opponent's land count with the controller's.
...
// Audit: metadata-only — Needs flanking plus a one-point damage-redirection shield whose destination is chosen as the ability resolves.
...
// VIS 27 — Breezekeeper
// Audit: metadata-only — Automatic phasing during the untap procedure is not represented as an executable keyword ability.
...
// VIS 28 — Chronatog
// Audit: metadata-only — Needs a per-object activation quota combined with an effect that skips the activating player's next turn.
...
// VIS 30 — Desertion
// Audit: metadata-only — The shared counter effect cannot route a countered artifact or creature spell directly onto the battlefield under the countering player's control.
...
// VIS 31 — Dream Tides
// Audit: metadata-only — Needs an upkeep choice of any number of tapped nongreen creatures with one atomic {2}-per-chosen-object payment and selective untapping.
...
// Audit: metadata-only — Activated costs cannot yet return two chosen battlefield permanents to their owners' hands.
...
// VIS 33 — Foreshadow
// Audit: metadata-only — Needs the milled card bound across a chosen-name comparison before the conditional draw, while retaining the separate delayed draw.
...
// Audit: metadata-only — Its enters payment and destruction are expressible, but flanking is not yet a removable keyword ability.
...
// VIS 39 — Ovinomancer
// Audit: metadata-only — The enters clause needs an atomic payment returning three chosen basic lands from the battlefield to hand.
...
// Audit: metadata-only — Needs automatic phasing plus a trigger event for this permanent phasing in.
...
// Audit: metadata-only — Needs an event-player type choice followed by a nontoken mass phase-out with normal phase-in ownership.
...
// VIS 45 — Three Wishes
// Audit: metadata-only — Needs linked face-down exile, private look access, play permission through the next turn, and cleanup of only the unplayed cards.
...
// Audit: metadata-only — Needs one simultaneous operation that phases in every phased-out creature while phasing out every creature with phasing.
...
// VIS 55 — Crypt Rats
// Audit: metadata-only — Variable activation costs cannot yet restrict every mana spent on X to black mana.
...
// VIS 57 — Death Watch
// Audit: metadata-only — Needs last-known power, toughness, and controller values from the enchanted creature that died.
...
// VIS 58 — Desolation
// Audit: metadata-only — Needs per-player land-mana history, APNAP sacrifice choices, and a linked check for whether each sacrificed land was a Plains.
...
// Audit: metadata-only — The blocking prohibition is available, but flanking is not yet a removable keyword ability.
...
// Audit: metadata-only — Needs a repeatable resolution loop coupling each controller sacrifice to an opponent choice between sacrifice, discard, or life loss.
...
// Audit: metadata-only — Spell costs cannot yet return a chosen-X set of battlefield permanents to hand, though divided X damage is available.
...
// Audit: metadata-only — Additional costs cannot yet atomically sacrifice every permanent the caster controls and discard that player's entire hand.
...
// VIS 65 — Necrosavant
// Audit: metadata-only — A graveyard-source activation cannot yet pay a sacrifice cost chosen from the battlefield.
...
pub(in crate::card::sets) static PILLAR_TOMBS_OF_AKU: CardRecord = CardRecord::new(
    ...
    CardRules::new_enchantment(mana_cost!("{2}{B}{B}"))
        ...
        .with_ability(AbilityDef::triggered(
            ...
            EffectDef::PayOr(PayOrDef::unless(
                EffectPaymentDef {
                    payer: PlayerSetDef::One(PlayerRefDef::EventPlayer),
...
// Audit: metadata-only — Fear can be approximated as a blocking rule, but is not yet represented as a removable granted keyword ability.
...
pub(in crate::card::sets) static TAR_PIT_WARRIOR: CardRecord = CardRecord::new(
    ...
    CardRules::new_creature(mana_cost!("{2}{B}"), &["Cyclops", "Warrior"], 3, 4).with_ability(
        AbilityDef::triggered(
            ...
            TriggerEventDef::BecomesTargetOfSpellOrAbility(ObjectPredicateDef::Source),
            EffectDef::Sacrifice {
                object: EffectRecipientDef::Source,
            },
        ),
...
pub(in crate::card::sets) static WAKE_OF_VULTURES: CardRecord = CardRecord::new(
    ...
    crate::card::CardSet::Visions,
    CardRules::new_creature(mana_cost!("{3}{B}"), &["Bird"], 3, 1).with_abilities(&[
        abilities::flying(),
...
// VIS 78 — Elkin Lair
// Audit: metadata-only — Needs random hand exile linked to temporary play permission and an end-step move only when that exact card was not played.
...
// Audit: metadata-only — The search must accept an unbounded ordered set, shuffle first, then preserve the chosen order on top of the library.
...
// VIS 83 — Heat Wave
// Audit: metadata-only — Needs a repeated colored cumulative-upkeep payment and a blocking declaration cost of 1 life per nonblue blocker.
        ...
        AbilityDef::triggered_if(
            ...
            &TriggerConditionDef::ObjectCount {
                query: ObjectQueryDef::matching(
                    ObjectPredicateDef::All(&[
                        ObjectPredicateDef::HasType(CardType::Creature),
                        ObjectPredicateDef::Named("Keeper of Kookus"),
                    ]),
                    &[ZoneKind::Battlefield],
                    PlayerRelation::You,
                ),
...
// Audit: metadata-only — Needs an attack-declaration trigger conditioned on every controlled non-Wall creature attacking, then a frozen attacker count.
...
// Audit: metadata-only — Needs lethal-damage state grouped by individual source rather than total damage marked on the creature.
...
// Audit: metadata-only — Needs the four-card mill result retained as a creature-card count for a turn-long trigger on every later attacker.
...
// Audit: metadata-only — Haste is available, but flanking is not yet a removable keyword ability.
...
pub(in crate::card::sets) static TREMOR: CardRecord = CardRecord::new(
    ...
    CardRules::new_sorcery(mana_cost!("{R}")).with_ability(AbilityDef::spell(
        "This spell deals 1 damage to each creature without flying.",
        EffectDef::DealDamage {
            recipient: EffectRecipientDef::matching_objects(
                ObjectPredicateDef::All(&[
                    ObjectPredicateDef::HasType(CardType::Creature),
...
// Audit: metadata-only — Resolving payments can return only one matching permanent, not two chosen Forests atomically.
...
// Audit: metadata-only — Player play and activation restrictions cannot yet be conditioned continuously on whose turn it is.
...
// Audit: metadata-only — Automatic phasing is missing, and the flying-creature lock also needs a tap-symbol activation prohibition.
    ...
    CardRules::new_creature(mana_cost!("{3}{G}"), &["Drake"], 2, 2).with_abilities(&[
        ...
        AbilityDef::activated(
            ...
            &[
                AbilityCostDef::SacrificeSource,
                AbilityCostDef::SacrificePermanent {
                    object: ObjectPredicateDef::Named("Spitting Drake"),
                    controller: PlayerRelation::You,
                },
            ],
            EffectDef::SearchZone {
                player: EffectRecipientDef::Controller,
                source: ZoneKind::Library,
                object: ObjectPredicateDef::Named("Viashivan Dragon"),
                minimum: 0,
                maximum: ValueDef::Constant(1),
                reveal: false,
...
// Audit: metadata-only — Needs a damage replacement that prevents the event and converts its exact amount into that many -1/-1 counters.
        ...
        .with_abilities(&[
            ...
            AbilityDef::triggered(
                ...
                TriggerEventDef::DamageDealt(crate::card::DamageEventMatcherDef {
                    ...
                    ..crate::card::DamageEventMatcherDef::ANY
                }),
                EffectDef::Destroy {
                    object: EffectRecipientDef::AttachedPermanent,
                    can_regenerate: true,
...
// Audit: metadata-only — Activated costs cannot yet return a chosen battlefield permanent to its owner's hand.
...
// VIS 119 — Rowen
// Audit: metadata-only — Needs replacement handling for the first draw each turn that reveals and binds the drawn card for the basic-land trigger.
...
// Audit: metadata-only — Extra-land permissions are currently valid only as static abilities, not as a resolving effect lasting until end of turn.
...
// Audit: metadata-only — Needs a draw replacement that reveals and binds the drawn card before a per-player creature-card discard-or-life payment.
...
// VIS 128 — Corrosion
// Audit: metadata-only — Needs per-artifact rust-counter thresholds after a mass counter placement plus cleanup of every rust counter when the source leaves.
...
// Audit: metadata-only — Cumulative upkeep cannot yet multiply the compound colored cost {U}{R} by the number of age counters.
...
// Audit: metadata-only — The existing graveyard selector finds the top matching creature rather than testing whether the graveyard's actual top card is a creature.
...
// VIS 132 — Mundungu
// Audit: metadata-only — Resolving payments currently offer one mana or life cost, not the compound {1} and 1 life payment this counter effect requires.
...
// VIS 133 — Pygmy Hippo
// Audit: metadata-only — Needs forced mana-ability activation, loss measurement for the defender's mana pool, and a delayed main-phase mana award.
...
// Audit: metadata-only — Mana production cannot yet derive the set of mana types from the land sacrificed as the activation cost.
...
pub(in crate::card::sets) static MAGMA_MINE: CardRecord = CardRecord::new(
    ...
    CardRules::new_artifact(mana_cost!("{1}")).with_abilities(&[
        ...
        AbilityDef::activated_with_targets(
            ...
            EffectDef::DealDamage {
                recipient: EffectRecipientDef::Target(TargetIndex::PRIMARY),
                amount: ValueDef::CountersOnSource(CounterKind::named("pressure")),
            },
        ),
    ]),
);
...
// Audit: metadata-only — Regeneration shields cannot yet attach a follow-up that adds a -1/-1 counter only when that specific shield is used.
...
// Audit: metadata-only — Needs chosen X preserved from casting into an enters-with-counters replacement and a dynamic attack declaration payment.
...
// Audit: metadata-only — Needs a global untap-step skip plus one simultaneous tapped-state inversion over each active player's artifacts, creatures, and lands.
...
// Audit: metadata-only — Needs an ordered snapshot of the event player's hand moved to the library bottom and its frozen size reused for the draw.
...
// Audit: metadata-only — Needs private inspection of a target player's actual top library card, a nonland check, and a life payment linked to moving that card.
...
// Audit: metadata-only — Needs a mana-ability rider that returns the source during its controller's next untap procedure as it untaps.
