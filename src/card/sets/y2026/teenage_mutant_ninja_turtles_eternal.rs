...
AbilityDef::triggered("Whenever Shredder attacks a player, for each other opponent, create a token that's a copy of Shredder tapped and attacking that player, except it isn't legendary. Sacrifice those tokens at end of combat.", TriggerEventDef::attacks_a_player(ObjectPredicateDef::Source), EffectDef::None),
...
abilities::dies_trigger("When this creature dies, create a number of 1/1 colorless Robot artifact creature tokens equal to the number of +1/+1 counters on this creature.", EffectDef::CreateToken(crate::card::CreateTokenDef::new(crate::card::TokenDef::Literal(crate::card::TokenCharacteristics::artifact_creature(&["Robot"], &[], 1, 1))).with_count(ValueDef::CountersOnSource(CounterKind::PlusOnePlusOne))))
]),
);
