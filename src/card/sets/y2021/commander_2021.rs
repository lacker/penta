...
pub(in crate::card::sets) static STINGING_STUDY: CardRecord = CardRecord::new(
    "Stinging Study",
    "b8840226-1693-44bc-a067-e50198c5e17e",
    "Kieran Yanner",
    CardRules::new_instant(mana_cost!("{4}{B}")).with_abilities(&[AbilityDef::spell(
        "You draw X cards and you lose X life, where X is the mana \
         value of a commander you own on the battlefield or in the \
         command zone.",
        EffectDef::Choose(ChooseDef {
            ...
            minimum: 1,
            maximum: 1,
            binding: ObjectChoiceBindingDef::Objects(Binding!("study_commander")),
            unchosen: None,
            visibility: ChoiceVisibilityDef::Private,
            then: &EffectDef::ForEachInBinding {
                objects: Binding!("study_commander"),
                binding: Binding!("studied_commander"),
                effect: &EffectDef::Sequence(&[
                    EffectDef::DrawCards {
                        recipient: EffectRecipientDef::Controller,
                        amount: ValueDef::ObjectManaValue(ObjectRefDef::Binding(Binding!(
                            "studied_commander"
                        ))),
                    },
                    EffectDef::LoseLife {
...
pub(in crate::card::sets) static ANGEL_OF_THE_RUINS: CardRecord = CardRecord::new(
    "Angel of the Ruins",
    "b2babb93-6b30-4446-bf51-b0303e2e9a27",
    "Viko Menezes",
    CardRules::new_creature(mana_cost!("{5}{W}{W}"), &["Angel"], 5, 7)
        .with_abilities(&[
            abilities::flying(),
...
// Audit: unsupported — CopyEntering has no duration. A resolving BecomeCopyOf until end of turn
// would occur after entry and would miss the copied creature's entry replacement and triggered
// abilities.
pub(in crate::card::sets) static CURSED_MIRROR: CardRecord = CardRecord::new(
    "Cursed Mirror",
...
pub(in crate::card::sets) static RIONYA_FIRE_DANCER: CardRecord = CardRecord::new(
    "Rionya, Fire Dancer",
    "536c2936-2fdc-4664-97fd-bd580fc2d90e",
    "Heonhwa",
    CardRules::new_creature(mana_cost!("{3}{R}{R}"), &["Human", "Wizard"], 3, 4)
        .with_supertype(CardSupertype::Legendary)
        .with_abilities(&[AbilityDef::triggered_with_targets(
            "At the beginning of combat on your turn, create X tokens \
             that are copies of another target creature you control, \
             where X is one plus the number of instant and sorcery \
             spells you've cast this turn. They gain haste. Exile them \
            ...
            EffectDef::CreateToken(
                ...
                .with_count(ValueDef::Sum(&SumValueDef {
                    left: ValueDef::Constant(1),
                    right: ValueDef::CountSpellsCastThisTurn(&SpellCastQueryDef {
